...
gender_education
```
`par()` doesn't work because that is in bbase r using graphs, these bad boys are grobs (lmao) so you need to use another package {gridExtra}
...
Totally fine to just observe the plots!
...
```{r compare}
par(mfrow = c(1, 2))
hist(zombies$zombies_killed, freq = F, probability = T)
hist(zombies$years_of_education, freq = F, probability = T)
hist(rpois(1000,3), probability = T)
```
This will help compare the histograms to show poisson distribution
...
for (i in 1:1000) { #the example in mod 7 uses 10000 and i can't see a reason to change it?? change it so its more like the original sample size
...
eduset <- NULL
n <- 30 #adjusted to our original n
for (i in 1:1000) { #changed to 1000 see above
...
3. For the ggplots I was just picking ones that didn't require extra information so I'm not sure if they are the best suited, but I don' really understand the extra arguments for some of the geom_functions.
